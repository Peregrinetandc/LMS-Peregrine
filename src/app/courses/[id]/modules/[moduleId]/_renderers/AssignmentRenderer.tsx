    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        <div className="rounded-lg border border-slate-200 bg-white px-4 py-3">
          ...
          <p className="mt-1 text-lg font-bold text-slate-900">{assignmentRow.max_score ?? '—'}</p>
        </div>
        <div className="rounded-lg border border-slate-200 bg-white px-4 py-3">
          <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Passing score</p>
          <p className="mt-1 text-lg font-bold text-slate-900">{assignmentRow.passing_score ?? '—'}</p>
        </div>
      </div>
      {assignmentRow.deadline_at && (
        <div className="rounded-lg border border-amber-200 bg-amber-50/70 px-4 py-3 text-sm text-amber-900">
          ...
          <p className="mt-1 font-medium">{formatLocalDisplay(assignmentRow.deadline_at)}</p>
        </div>
      )}
