-- Replace dashboard_learner_summary_v1 to:
--   1) Add `due_assignments_count` — uncapped total used by the KPI tile, so
--      learners with >10 open assignments see the true number.
--   2) Trim the inline `due_assignments` list to LIMIT 3 — the dashboard only
--      renders the top 3 now; the dedicated /dashboard/due-assignments page
--      handles the full list via learner_due_assignments_v1.
--
-- Everything else (enrolled_courses, streak) is unchanged from
-- 20260504140000_dashboard_summary_typeaware_progress.sql.
